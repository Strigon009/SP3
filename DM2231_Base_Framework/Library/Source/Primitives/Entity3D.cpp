...
bool CEntity3D::CheckForCollision(const CEntity3D* cEntity3D)
{
	...
	if (
		this->eType == PROJECTILE && cEntity3D->eType == PLAYER ||
		this->eType == PLAYER && cEntity3D->eType == PROJECTILE
		)
	{
		return false;
	}

	glm::vec3 rPos = glm::abs(cEntity3D->vec3Position - vec3Position);
	glm::vec3 rScale = glm::abs(cEntity3D->vec3ColliderScale + vec3ColliderScale) * 0.5f;
	...
	{
		if (this->eType == PLAYER)
		{
			vec3Position = GetPreviousPosition();
		}
