...
bool CScene3D::Init(void)
{
	...
	CEnemyBoss3D* ce = new CEnemyBoss3D;
	AddBoss1(ce, glm::vec3(10, 0.5, 10), glm::vec3(1, 1, 1));
...
void CScene3D::Update(const double dElapsedTime)
{
	...
	if (wave_count == 1)
	{
		if (zombieCount <= 11)
		{
			if (spawnZTimer >= 1.4f)
			{
				...
				AddEnemy2(cEnemy3D2, set_enemySpawnPos(), glm::vec3(0.5, 0.5, 0.5));
	...
	if (wave2_start == true)
	{
		...
		if (crawlerCount != 6)
		{
			if (spawnCTimer >= 1.4f)
			{
				...
				AddEnemy(cEnemy3D, set_enemySpawnPos(), glm::vec3(0.5, 0.5, 0.5));
	...
	if (wave3_start == true)
	{
		...
		if (scrakeCount != 4)
		{
			if (spawnSTimer >= 1.4f)
			{
				...
				AddEnemy3(cEnemy3D, set_enemySpawnPos(), glm::vec3(0.5, 0.5, 0.5));
	...
	if (boss_start == true)
	{
		...
		if (bossDED == false)
		{
			...
			if (bossStageLimit <= 42)
			{
				if (spawnZTimer >= 2.2f)
				{
					...
					case 3:
					{
						CEnemy3D2* cEnemy3D = new CEnemy3D2();
						AddEnemy2(cEnemy3D, set_enemySpawnPos(), glm::vec3(0.5, 0.5, 0.5));
					}
					}
		...
		if (bossCount <= 0)
		{
			AddBoss1(cBoss, set_enemySpawnPos(), glm::vec3(0.1, 0.1, 0.1));
			++bossCount;
		}
